...
1. （差し替える元の）空欄画像の表示されているタグをノードとして取得する
...
| どこに   | 空欄画像の表示されているタグに | querySelector関数                       |
| 何をする | スタイルを書き換える           | ノードのstyle.backgroundImageプロパティ |

特定のHTMLタグのノードを取得するので、<ruby><rb>querySelector</rb><rt>クエリ セレクタ</rt></ruby>関数を使いましょう。
querySelector関数は、引数にCSSでタグを指定するのと同じ方法で、特定のタグをノードとして
取得する関数です。
